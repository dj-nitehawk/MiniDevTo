...
public class Tests(Fixture App) : TestBase<Fixture>
{
    ...
    public async Task Invalid_Article_Data()
    {
        ...
        var (rsp, res) = await App.Client.POSTAsync<Save.Endpoint, Save.Request, ErrorResponse>(req);
    ...
    public async Task Create_New_Articles()
    {
        for (var i = 0; i < 3; i++)
        {
            var req = new Save.Request
            {
                Content = Fake.Lorem.Paragraphs(),
                Title = Fake.Hacker.Phrase()
            };

            var (rsp, res) = await App.Client.POSTAsync<Save.Endpoint, Save.Request, Save.Response>(req);

            rsp.IsSuccessStatusCode.Should().BeTrue();
            res.Message.Should().Be("Article saved!");
            res.ArticleID.Should().NotBeNullOrEmpty();

            App.ArticleIDs.Add(res.ArticleID!);
        }

        App.ArticleIDs.Count.Should().Be(3);
    }

    [Fact, Priority(2)]
    public async Task Get_Article_List()
    {
        var (rsp, res) = await App.Client.GETAsync<List.Endpoint, IEnumerable<List.Article>>();

        rsp.IsSuccessStatusCode.Should().BeTrue();
        res.Count().Should().Be(3);
        res.Select(a => a.ArticleID).Should().Equal(App.ArticleIDs);
    }

    [Fact, Priority(3)]
    public async Task Get_Article()
    {
        var articleID = App.ArticleIDs[0];

        var (rsp1, res1) = await App.Client.GETAsync<Get.Endpoint, Get.Request, Get.Response>(
                               new()
                               {
                                   ArticleID = articleID
                               });
        ...
        var (rsp2, _) = await App.Client.GETAsync<Get.Endpoint, Get.Request, Get.Response>(
                            new()
                            {
    ...
    public async Task Delete_Article()
    {
        ...
        var (rsp, res) = await App.Client.DELETEAsync<Delete.Endpoint, Delete.Request, Delete.Response>(
                             new()
                             {
                                 ArticleID = articleID
                             });
        ...
        var exists = await DB.Find<Dom.Article>()
                             .MatchID(articleID)
                             .ExecuteAnyAsync();
